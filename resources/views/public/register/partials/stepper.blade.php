...
<div class="mb-6">
    <div class="relative mx-auto max-w-xl mb-12">
            ...
            @foreach ($steps as $num => $label)
                ...
                <div class="flex flex-col items-center gap-2">
                    <div class="flex h-10 w-10 items-center justify-center rounded-full border text-sm font-semibold
                            {{ $active ? 'border-[#d9b76f]/70 bg-[#d9b76f] text-black' : ($done ? 'border-[#d9b76f]/60 bg-[#3d453d] text-[#f3e2b6]' : 'border-[#d9b76f]/25 bg-[#3d453d] text-neutral-200') }}"
                        title="{{ $label }}">
                        @if ($done)
                            ✓
                        @else
