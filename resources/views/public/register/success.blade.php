...
@section('content')
    ...
    <div class="guest-infor mt-2 rounded-xl border p-6 text-center">
        <div>
            <span class="text-xs tracking-[0.2em] text-neutral-200/75">TỔNG SỐ KHÁCH: </span>
            <span class=" text-2xl font-semibold text-neutral-100">{{ $registration->total_count }}</span>
        </div>
        <div>
            <span class="text-xs tracking-[0.2em] text-neutral-200/75">THỜI GIAN:</span>
            <span class="text-lg font-semibold text-[#d9b76f]">
                    {{ $registration->eventSession->starts_at->format('d/m/Y H:i') }}
            </span>
        </div>
        <div class='mt-2'>
            <span class="text-xs tracking-[0.2em] text-neutral-200/75">ĐỊA ĐIỂM:</span>
            <div class="text-lg font-semibold text-[#d9b76f]">{{ $registration->eventSession->venue->name }}</div>
                @if ($registration->eventSession->venue->address)
                    <div class="text-sm text-neutral-200/80">{{ $registration->eventSession->venue->address }}</div>
                @endif
        </div>
